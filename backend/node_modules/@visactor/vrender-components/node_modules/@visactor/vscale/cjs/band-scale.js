...
const vutils_1 = require("@visactor/vutils"), ordinal_scale_1 = require("./ordinal-scale"), utils_1 = require("./utils/utils"), type_1 = require("./type"), tick_sample_int_1 = require("./utils/tick-sample-int");

class BandScale extends ordinal_scale_1.OrdinalScale {
    constructor(slience) {
        super(), this.type = type_1.ScaleEnum.Band, this._range = [ 0, 1 ], this._step = void 0, 
    ...
    rescale(slience, changeProperty) {
        ...
        const stop = wholeRange[1 - Number(reverse)], space = (0, utils_1.bandSpace)(n, this._paddingInner, this._paddingOuter);
    ...
    scale(d) {
        ...
        if (!i) {
            if (this._unknown !== ordinal_scale_1.implicit) return this._unknown;
    ...
    _calculateWholeRange(range, changeProperty) {
        ...
        if (this.isBandwidthFixed()) {
            const wholeLength = (0, utils_1.scaleWholeRangeSize)(super.domain().length, this._bandwidth, this._paddingInner, this._paddingOuter) * Math.sign(range[1] - range[0]), rangeFactorSize = Math.min((range[1] - range[0]) / wholeLength, 1);
    ...
    range(_, slience) {
        return _ ? (this._range = [ (0, vutils_1.toNumber)(_[0]), (0, vutils_1.toNumber)(_[1]) ], 
        this.rescale(slience)) : this._range;
    }
    rangeRound(_, slience) {
        return this._range = [ (0, vutils_1.toNumber)(_[0]), (0, vutils_1.toNumber)(_[1]) ], 
        this._round = !0, this.rescale(slience);
    }
    ticks(count = 10) {
        const d = this.calculateVisibleDomain(this._range);
        if (-1 === count) return d;
        return (0, tick_sample_int_1.ticks)(0, d.length - 1, count, !1).map((i => d[i]));
    }
    tickData(count = 10) {
    ...
    forceTicks(count = 10) {
        const d = this.calculateVisibleDomain(this._range);
        return (0, tick_sample_int_1.ticks)(0, d.length - 1, count, !0).filter((i => i < d.length)).map((i => d[i]));
    }
    stepTicks(step) {
        const d = this.calculateVisibleDomain(this._range);
        return (0, tick_sample_int_1.stepTicks)(0, d.length - 1, step).map((i => d[i]));
    }
    _getInvertIndex(d) {
        let i = 0;
        ...
        for (i = 0; i < len; i++) {
            ...
            if (0 === i && (!reverse && !(0, vutils_1.isGreater)(d, r + halfStep) || reverse && !(0, 
            vutils_1.isLess)(d, r - halfStep))) break;
            if (i === len - 1) break;
            if (!(0, vutils_1.isLess)(d, r - halfStep) && !(0, vutils_1.isGreater)(d, r + halfStep)) break;
        }
    ...
    _isBandwidthFixedByUser() {
        return this._isFixed && this._userBandwidth && (0, vutils_1.isNumber)(this._userBandwidth);
    }
    clone() {
        var _a, _b, _c;
