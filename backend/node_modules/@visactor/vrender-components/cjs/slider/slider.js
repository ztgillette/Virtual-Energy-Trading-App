...
const vutils_1 = require("@visactor/vutils"), vrender_core_1 = require("@visactor/vrender-core"), base_1 = require("../core/base"), constant_1 = require("./constant"), register_1 = require("./register"), event_1 = require("../util/event");

function convertValueToRange(value) {
    return (0, vutils_1.isArray)(value) ? value : [ value, value ];
}
...
(0, register_1.loadSliderComponent)();

class Slider extends base_1.AbstractComponent {
    get track() {
        return this._track;
    }
    get currentValue() {
    ...
    render() {
        ...
        innerView.name = constant_1.SLIDER_ELEMENT_NAME.innerView, this.add(innerView), 
        this._innerView = innerView;
        let startTextShape, startLen = 0;
        if (startText && startText.visible) {
            startTextShape = vrender_core_1.graphicCreator.text(Object.assign({
            ...
            }, startText.style)), startTextShape.name = constant_1.SLIDER_ELEMENT_NAME.startText, 
            innerView.add(startTextShape);
            const space = (0, vutils_1.isValid)(startText.space) ? startText.space : 0;
            startLen += (isHorizontal ? startTextShape.AABBBounds.width() : startTextShape.AABBBounds.height()) + space;
        }
        const mainContainer = vrender_core_1.graphicCreator.group({
            x: isHorizontal ? startLen : 0,
            y: isHorizontal ? 0 : startLen
        });
        innerView.add(mainContainer);
        const railContainer = vrender_core_1.graphicCreator.group({
            x: 0,
            y: 0
        });
        let endTextShape;
        ...
        startLen += isHorizontal ? railWidth : railHeight, endText && endText.visible) {
            ...
            }, endText.style)), endTextShape.name = constant_1.SLIDER_ELEMENT_NAME.endText, 
    ...
    _renderRail(container) {
        ...
        const railShape = vrender_core_1.graphicCreator.rect(Object.assign({
            x: 0,
            y: 0,
            width: railWidth,
            height: railHeight,
        ...
        return railShape.name = constant_1.SLIDER_ELEMENT_NAME.rail, container.add(railShape), 
        this._rail = railShape, railShape;
    }
    ...
    _renderTrack(container) {
        ...
        if ((0, vutils_1.isNil)(value)) range ? (startValue = min, endValue = max) : startValue = endValue = min; else if (range) {
        ...
        trackContainer.name = constant_1.SLIDER_ELEMENT_NAME.trackContainer;
    ...
    _renderHandler(style) {
        return vrender_core_1.graphicCreator.symbol(style);
    }
    ...
    _bindEvents() {
        ...
        this._railContainer.addEventListener("pointerdown", this._onRailPointerDown), ("browser" === vrender_core_1.vglobal.env ? vrender_core_1.vglobal : this.stage).addEventListener("touchmove", this._handleTouchMove, {
            passive: !1
        }));
    }
    _bindTooltipEvents() {
    ...
    _updateHandler(handler, position, value) {
        ...
        const updateHandlerText = handler.name === constant_1.SLIDER_ELEMENT_NAME.startHandler ? this._startHandlerText : this._endHandlerText;
        ...
        handler.name === constant_1.SLIDER_ELEMENT_NAME.startHandler ? (this._currentValue.startValue = value, 
    ...
    _updateHandlerText(handlerText, position, value) {
        ...
        const updateHandler = handlerText.name === constant_1.SLIDER_ELEMENT_NAME.startHandlerText ? this._startHandler : this._endHandler;
        updateHandler && updateHandler.setAttributes({
            [isHorizontal ? "x" : "y"]: position
        }), handlerText.name === constant_1.SLIDER_ELEMENT_NAME.startHandlerText ? (this._currentValue.startValue = value, 
        this._currentValue.startPos = position) : (this._currentValue.endValue = value, 
        this._currentValue.endPos = position);
    }
...
exports.Slider = Slider, Slider.defaultAttributes = {
    slidable: !0,
    layout: "horizontal",
    align: "bottom",
    height: 8,
