...
function colorArrayToString(color, alphaChannel = !1) {
    return Array.isArray(color) && (0, vutils_1.isNumber)(color[0]) ? alphaChannel ? `rgb(${Math.round(color[0])},${Math.round(color[1])},${Math.round(color[2])},${color[3].toFixed(2)})` : `rgb(${Math.round(color[0])},${Math.round(color[1])},${Math.round(color[2])})` : color;
}

function interpolateColor(from, to, ratio, alphaChannel, cb) {
    if (Array.isArray(from) && !(0, vutils_1.isNumber)(from[0]) || Array.isArray(to) && !(0, 
    vutils_1.isNumber)(to[0])) {
        return new Array(4).fill(0).map(((_, index) => {
            var _a, _b;
            return _interpolateColor((0, vutils_1.isArray)(from) ? null !== (_a = from[index]) && void 0 !== _a ? _a : from[0] : from, (0, 
            vutils_1.isArray)(to) ? null !== (_b = to[index]) && void 0 !== _b ? _b : to[0] : to, ratio, alphaChannel);
        }));
    }
