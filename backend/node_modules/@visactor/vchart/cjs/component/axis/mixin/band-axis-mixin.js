...
const vutils_1 = require("@visactor/vutils"), event_1 = require("../../../constant/event"), compilable_data_1 = require("../../../compile/data/compilable-data"), util_1 = require("../util");
    ...
    _initData() {
        ...
        if (this._spec.showAllGroupLayers && this._scales.length > 1) for (let layer = 0; layer < this._scales.length; layer++) {
            ...
            if (!1 !== layerConfig.visible && 0 !== layerConfig.tickCount && 0 !== layerConfig.forceTickCount) {
                const tickData = this._initTickDataSet(Object.assign(Object.assign({}, this._tickTransformOption()), layerConfig), layer), compilableData = new compilable_data_1.CompilableData(this._option, tickData);
            ...
            const tickData = this._initTickDataSet(this._tickTransformOption()), compilableData = new compilable_data_1.CompilableData(this._option, tickData);
        ...
        return (this._spec.showAllGroupLayers ? this._scales : [ this._scales[0] ]).forEach(((scale, index) => {
            ...
            if (ticks && ticks.length) if (preData && preData.length) {
                ...
                preData.forEach((value => {
                    ticks.forEach((tick => {
                        const values = (0, vutils_1.array)(value).concat(tick);
                        if (curData.push(values), isTickDataHaveData) {
                            const axisItem = (0, util_1.getAxisItem)(tick, this._getNormalizedValue(values, length));
                ...
                preData.push(tick);
            })), isTickDataHaveData && labelItems.push(tickData.getLatestData().map((obj => (0, 
            util_1.getAxisItem)(obj.value, this._getNormalizedValue([ obj.value ], length)))).filter((entry => entry.value >= 0 && entry.value <= 1)));
        })), labelItems.reverse();
    }
    _updateRawDomain() {
        this._rawDomainIndex = [];
