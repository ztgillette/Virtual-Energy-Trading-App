...
const resolveTooltipFilterRange = (spec, scale) => {
    ...
    return (0, vutils_1.isValidNumber)(rangeValue) ? [ -rangeValue, rangeValue ] : rangeValue;
}, getDimensionData = (value, axis, coordinate, getDimensionField) => {
    var _a;
    const scale = axis.getScale(), isDiscreteAxis = (0, vscale_1.isDiscrete)(scale.type), data = [], seriesList = axis.getOption().getChart().getSeriesInIndex(axis.getSpecInfo().seriesIndexes);
    for (const series of seriesList) if (series.coordinate === coordinate) {
        ...
        } else if ((0, vutils_1.isValid)(dimensionField[1])) {
            const datums = [], datumIdList = [];
            viewData.forEach(((datum, i) => {
                var _a;
                ((null === (_a = datum[dimensionField[0]]) || void 0 === _a ? void 0 : _a.toString()) === (null == value ? void 0 : value.toString()) || (0, 
                vutils_1.isValid)(datum[dimensionField[0]]) && (0, vutils_1.isValid)(datum[dimensionField[1]]) && value >= datum[dimensionField[0]] && value < datum[dimensionField[1]]) && (datums.push(datum), 
                datumIdList.push(i));
            })), data.push({
                series: series,
                datum: datums,
...
const getDimensionDataKey = (series, datumIdList) => `${series.id}_${datumIdList.join("_")}`, getAxis = (chart, filter, pos) => {
