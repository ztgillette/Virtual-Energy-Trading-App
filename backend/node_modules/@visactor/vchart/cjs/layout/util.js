...
Object.defineProperty(exports, "__esModule", {
    value: !0
}), exports.getItemLayoutWithTag = exports.layoutBottomInlineItems = exports.layoutTopInlineItems = exports.layoutRightInlineItems = exports.layoutLeftInlineItems = void 0;
...
function getPositionItems(items) {
    ...
    return items.forEach((item => {
        (0, vutils_1.isNil)(item.getSpec().position) || "start" === item.getSpec().position ? startItems.push(item) : "middle" === item.getSpec().position ? middleItems.push(item) : "end" === item.getSpec().position && endItems.push(item);
    })), {
...
function adjustItemsToCenter(allItems, isVertical, containerLength) {
    isVertical ? allItems.forEach((items => {
        const lastItem = (0, vutils_1.last)(items), length = lastItem.getLayoutStartPoint().y + lastItem.getLayoutRect().height - items[0].getLayoutStartPoint().y, centerY = (containerLength - length) / 2;
        items.forEach((item => {
        ...
        const lastItem = (0, vutils_1.last)(items), length = lastItem.getLayoutStartPoint().x + lastItem.getLayoutRect().width - items[0].getLayoutStartPoint().x, centerX = (containerLength - length) / 2;
        items.forEach((item => {
            item.setLayoutStartPosition({
...
function layoutLeftInlineItems(items, layout, limitHeight, recompute) {
...
function layoutRightInlineItems(items, layout, limitHeight, recompute) {
    const {startItems: startItems, middleItems: middleItems, endItems: endItems} = getPositionItems(items);
...
function layoutTopInlineItems(items, layout, limitWidth, recompute) {
    const {startItems: startItems, middleItems: middleItems, endItems: endItems} = getPositionItems(items);
...
function layoutBottomInlineItems(items, layout, limitWidth, recompute) {
    const {startItems: startItems, middleItems: middleItems, endItems: endItems} = getPositionItems(items);
...
function getItemLayoutWithTag(item, layout, recompute, setRectToItem = !0) {
    ...
    if (layoutTag) {
        ...
        layoutRect = layout.getItemComputeLayoutRect(item), !item.willLayoutTag && (0, vutils_1.isEqual)(layoutRect, lastLayoutRect) ? (layoutTag = !1, 
        rect = item.getLayoutRect()) : (rect = item.computeBoundsInRect(layoutRect), (0, 
        vutils_1.isEqual)(lastRect[rectKey], rect[rectKey]) ? layoutTag = !1 : (setRectToItem && item.setLayoutRect(rect), 
...
exports.layoutLeftInlineItems = layoutLeftInlineItems, exports.layoutRightInlineItems = layoutRightInlineItems, 
exports.layoutTopInlineItems = layoutTopInlineItems, exports.layoutBottomInlineItems = layoutBottomInlineItems, 
exports.getItemLayoutWithTag = getItemLayoutWithTag;
