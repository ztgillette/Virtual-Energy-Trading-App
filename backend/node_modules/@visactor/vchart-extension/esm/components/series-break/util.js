...
const barOffset = 2, lineOffset = 10, areaOffset = 0;

export function getSeriesBreakConfig(axesSpec, axesIndex) {
    return {
        type: "component",
        ...
        style: {
            data: (datum, ctx) => {
                ...
                return axesSpec.forEach(((spec, index) => {
                    var _a, _b;
                    const axisId = spec.id, axisModel = isValid(axisId) ? chart.getComponentByUserId(axisId) : axesIndex && isValid(axesIndex[index]) ? chart.getComponentByIndex("axes", axesIndex[index]) : null === (_a = chart.getComponentsByKey("axes").filter((axis => axis.getSpec().breaks === spec.breaks))) || void 0 === _a ? void 0 : _a[0];
                    if (!axisModel) return;
                    const parsedAxisId = null != axisId ? axisId : `${axisModel.type}-${axisModel.id}`, regionBounds = getAllRegionBounds(axisModel.getRegions());
                    array(null === (_b = axisModel._break) || void 0 === _b ? void 0 : _b.breaks).forEach((breakConfig => {
...
function getIntersectionsFromLineAndPolyline(line, points) {
    ...
    for (let index = 1; index < points.length; index++) {
        ...
        intersection && !intersections.find((point => isNumberEqual(PointService.distancePP({
            x: point[0],
            y: point[1]
        }, {
            x: intersection[0],
...
export const appendSeriesBreakConfig = rawSpec => {
    var _a;
    if (null === (_a = rawSpec.axes) || void 0 === _a ? void 0 : _a.length) {
        ...
        if (breakedAxes.length) return rawSpec.customMark = array(rawSpec.customMark).filter((obj => obj.componentType !== SERIES_BREAK)), 
