...
export function groupBarsByFields(elements, groupFields) {
    ...
    for (let i = 0; i < elements.length; i++) {
        const item = elements[i], itemData = getDatumOfGraphic(item), groupKey = groupFields.map((field => itemData[field])).join("-");
...
export function getLinkData(currentElement, nextElement, config) {
    ...
    return isHorizontal ? (linePoints = [ {
        ...
        data: [ getDatumOfGraphic(currentElement), getDatumOfGraphic(nextElement) ],
                ...
                return regions.forEach((region => {
                    ...
                    if (barSeriesArr.length) {
                        ...
                        Object.values(groupData).forEach((groupedValues => {
                            groupedValues.sort(((prev, curr) => getDatumOfGraphic(prev)[STACK_FIELD_END] - getDatumOfGraphic(curr)[STACK_FIELD_END]));
                        }));
