...
import { last, array, get } from "@visactor/vutils";
...
export function dodge(positions, separation = 10, maxIter = 10, maxError = .1) {
...
export function dodgeHorizontal(texts, separation = 10, maxIter = 10) {
...
export function getSeriesLabelConfig(position, config) {
            ...
            data: (datum, ctx) => {
                ...
                if (1 === seriesTypes.length && [ "bar", "area", "line", "waterfall" ].includes(seriesTypes[0])) {
                    ("both-ends" === position ? [ "start", "end" ] : [ position ]).forEach((pos => {
                        const targetValue = "end" === pos ? last(bandAxisScale.domain()) : bandAxisScale.domain()[0], region = series.getRegion(), {x: regionStartX, y: regionStartY} = region.getLayoutStartPoint();
                        let i = 0;
...
export function appendSeriesLabelConfig(rawSpec, seriesLabelSpec) {
    var _a, _b, _c;
    if (rawSpec.customMark = array(rawSpec.customMark).filter((obj => obj.componentType !== SERIES_LABEL)), 
    seriesLabelSpec || (seriesLabelSpec = null !== (_c = null !== (_a = get(rawSpec, SERIES_LABEL)) && void 0 !== _a ? _a : get(null === (_b = rawSpec.series) || void 0 === _b ? void 0 : _b[0], SERIES_LABEL)) && void 0 !== _c ? _c : {}), 
