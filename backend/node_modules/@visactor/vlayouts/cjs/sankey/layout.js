...
Object.defineProperty(exports, "__esModule", {
    value: !0
}), exports.SankeyLayout = void 0;

const vutils_1 = require("@visactor/vutils"), hierarchy_1 = require("./hierarchy");
...
function center(node, maxDepth, nodeMap) {
    return node.targetLinks.length ? node.depth : node.sourceLinks.length ? (0, vutils_1.minInArray)(node.sourceLinks.map((link => nodeMap[link.target].depth))) - 1 : 0;
}

const ascendingNodeBreadth = (a, b) => (null == a ? void 0 : a.y0) - (null == b ? void 0 : b.y0), calcDivideValue = (node, isTarget) => {
    if ((0, vutils_1.isNil)(node.value)) return null;
    const res = (isTarget ? node.targetLinks : node.sourceLinks).reduce(((res, sLink) => ((0, 
    vutils_1.isNil)(sLink.value) ? res.count += 1 : res.sum += sLink.value, res)), {
        sum: 0,
        count: 0
    });
...
}, linkClampe = (0, vutils_1.clamper)(0, 1);
...
class SankeyLayout {
    constructor(options) {
        ...
        this._getNodeKey = keyFunc, this._logger = vutils_1.Logger.getInstance(), this._alignFunc = (0, 
        vutils_1.isFunction)(this.options.setNodeLayer) ? node => this.options.setNodeLayer(node.datum) : alignFunctions[this.options.nodeAlign];
    }
    layout(data, config) {
        if (!data) return null;
        ...
        if (this.computeLinkBreadths(nodes), nodes.forEach((node => {
            node.sourceLinks = node.sourceLinks.filter((link => !(0, vutils_1.isNil)(link.source) && !(0, 
            vutils_1.isNil)(link.target))), node.targetLinks = node.targetLinks.filter((link => !(0, 
            vutils_1.isNil)(link.source) && !(0, vutils_1.isNil)(link.target)));
        })), links = links.filter((link => !(0, vutils_1.isNil)(link.source) && !(0, vutils_1.isNil)(link.target))), 
    ...
    computeHierarchicNodeLinks(originalNodes) {
        return (0, hierarchy_1.computeHierarchicNodeLinks)(originalNodes, this._getNodeKey);
    }
    computeSourceTargetNodeLinks(data) {
        ...
        return data.links.forEach(((link, index) => {
            const hasSource = !(0, vutils_1.isNil)(link.source), hasTarget = !(0, vutils_1.isNil)(link.target);
            ...
            this.options.divideNodeValueToLink && (0, vutils_1.isNil)(link.value) && invalidLinks.push(linkElement), 
            ...
            const values = [ calcDivideValue(nodeMap[link.source]), calcDivideValue(nodeMap[link.target], !0) ].filter((entry => !(0, 
            vutils_1.isNil)(entry)));
            values.length && (link.value = (0, vutils_1.minInArray)(values));
        })), {
            nodeMap: nodeMap,
            nodes: nodes,
            links: links
    ...
    computeNodeValues(nodes) {
        return (0, hierarchy_1.computeNodeValues)(nodes);
    }
    computeNodeDepths(nodes) {
        var _a;
        const n = nodes.length;
        let next, nextMap, current = nodes, depth = 0, maxDepth = -1;
        const setNodeLayer = (0, vutils_1.isFunction)(this.options.setNodeLayer) ? this.options.setNodeLayer : null;
    ...
    computeNodeLayers(nodes) {
        ...
        if ((0, vutils_1.isString)(nodeWidthOption)) {
            const ratio = (0, vutils_1.clamp)(parseFloat(nodeWidthOption.replace("%", "")) / 100, 0, 1);
        ...
        } else if ((0, vutils_1.isNumber)(nodeWidthOption)) {
            if (nodeWidth = nodeWidthOption, (0, vutils_1.isNumber)(linkWidthOption)) linkWidth = linkWidthOption; else if ((0, 
            vutils_1.isNil)(linkWidthOption)) {
            ...
            isEvenWidth = !0;
        } else (0, vutils_1.isFunction)(nodeWidthOption) && (0, vutils_1.isNumber)(linkWidthOption) && (linkWidth = linkWidthOption);
        const columns = [];
        for (let i = 0, len = nodes.length; i < len; i++) {
            const node = nodes[i];
            node.layer = this._isHierarchic ? node.depth : (0, vutils_1.clamp)(Math.floor(this._alignFunc(node, this._maxDepth, this._nodeMap)), 0, this._maxDepth - 1);
        ...
        if (!isEvenWidth && (0, vutils_1.isFunction)(nodeWidthOption)) {
            ...
            for (let i = 0; i < this._maxDepth; i++) {
                ...
                for (let j = 0, colLen = column && column.length; j < colLen; j++) {
                    ...
                    for (let k = 0, linkLen = sourceLinks.length; k < linkLen; k++) {
                        const link = sourceLinks[k], curLinkWidth = (0, vutils_1.isFunction)(linkWidthOption) ? linkWidthOption(link, this._viewBox) : linkWidth;
    ...
    initializeNodeBreadths(columns) {
        ...
        ((0, vutils_1.isNil)(minNodeHeight) || minNodeHeight < minLinkHeight) && (minNodeHeight = minLinkHeight), 
        ((0, vutils_1.isNil)(maxLinkHeight) || maxLinkHeight > maxNodeHeight) && (maxLinkHeight = maxNodeHeight);
        let ky = 0, getGapY = null, forceNodeHeight = null;
        if ((0, vutils_1.isFunction)(this.options.nodeGap)) getGapY = this.options.nodeGap, 
        ...
        const isStartGap = "start" === this.options.gapPosition, isMiddleGap = !isStartGap && "end" !== this.options.gapPosition, getNodeHeight = (0, 
        vutils_1.isNumber)(this.options.nodeHeight) ? node => this.options.nodeHeight : (0, 
        vutils_1.isFunction)(this.options.nodeHeight) ? this.options.nodeHeight : forceNodeHeight > 0 ? node => forceNodeHeight : node => Math.max(node.value * ky, 0), getLinkHeight = (0, 
        vutils_1.isNumber)(this.options.linkHeight) ? () => this.options.linkHeight : (0, 
        vutils_1.isFunction)(this.options.linkHeight) ? this.options.linkHeight : (link, sourceNode, sourceNodeHeight) => Math.min(Math.max(sourceNode.value ? sourceNodeHeight * linkClampe(link.value / sourceNode.value) : 0, minLinkHeight, 0), maxLinkHeight);
    ...
    computeNodeBreadths(nodes) {
        ...
        if (!this._isHierarchic && !(0, vutils_1.isFunction)(this.options.setNodeLayer) && !(0, 
        vutils_1.isFunction)(this.options.nodeGap)) for (let i = 0; i < iterations; ++i) {
    ...
    relaxLeftToRight(columns, alpha, beta) {
        for (let i = 1, n = columns.length; i < n; ++i) {
            ...
            (0, vutils_1.isNil)(this.options.nodeSortBy) && column.sort(ascendingNodeBreadth), 
    ...
    reorderNodeLinks(node) {
        if ((0, vutils_1.isNil)(this.options.linkSortBy)) {
    ...
    reorderLinks(nodes) {
        if ((0, vutils_1.isNil)(this.options.linkSortBy)) for (let i = 0, len = nodes.length; i < len; i++) {
            const node = nodes[i];
...
exports.SankeyLayout = SankeyLayout, SankeyLayout.defaultOptions = {
    iterations: 6,
    nodeAlign: "justify",
