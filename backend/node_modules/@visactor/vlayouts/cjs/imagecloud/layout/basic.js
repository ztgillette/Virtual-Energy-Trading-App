...
const vutils_1 = require("@visactor/vutils"), util_1 = require("../util"), vscale_1 = require("@visactor/vscale"), image_1 = require("../../utils/image"), loader_1 = require("../../utils/loader");
...
class Layout {
    ...
    preProcess() {
        ...
        return images.forEach(((img, index) => {
            ...
            img.weight = (0, vutils_1.simpleField)(weight)(this.data[index]), img.datum = this.data[index]);
    ...
    loadSegmentationInput() {
        ...
        if (segmentationInput.boardSize = [ boardW, size[1] ], segmentationInput.random ? segmentationInput.randomGenerator = Math.random : segmentationInput.randomGenerator = (0, 
        vutils_1.fakeRandom)(), this.segmentationInput = segmentationInput, (0, vutils_1.isString)(segmentationInput.shapeUrl)) {
            segmentationInput.isEmptyPixel = (0, image_1.generateIsEmptyPixel)(void 0, {
            ...
            imagePromise ? (this.isMaskImageFinished = !1, this.isLayoutFinished = !1, imagePromise.then((shapeImage => {
                ...
                (null === (_a = options.maskConfig) || void 0 === _a ? void 0 : _a.removeWhiteBorder) && (0, 
                image_1.removeBorder)(shapeImage, maskCanvas, segmentationInput.isEmptyPixel);
                const shapeConfig = (0, image_1.scaleAndMiddleShape)(shapeImage, size);
                ...
                this.segmentationOutput = (0, image_1.segmentation)(this.segmentationInput), "masked" === (null === (_b = this.options.layoutConfig) || void 0 === _b ? void 0 : _b.placement) && (transparentMaskCanvas = this.generateTransparentMaskCanvas(shapeImage, size)), 
            ...
            const maskCanvas = (0, image_1.generateMaskCanvas)(segmentationInput.shapeUrl, size[0], size[1], void 0, null === (_f = options.maskConfig) || void 0 === _f ? void 0 : _f.invert, this.options.createCanvas);
            segmentationInput.maskCanvas = maskCanvas, this.segmentationOutput = (0, image_1.segmentation)(this.segmentationInput), 
            this.options.onUpdateMaskCanvas && this.options.onUpdateMaskCanvas(maskCanvas, maskCanvas), 
            this.isMaskImageFinished = !0;
        }
    }
    loadImageCollageInput() {
        const data = this.data, imagesPromise = (0, loader_1.loadImages)(data.map((d => (0, 
        vutils_1.simpleField)(this.options.image)(d))), this.options.createImage);
        imagesPromise ? imagesPromise.then((images => {
            this.onImageCollageInputReady(images), this.isImagesFinished = !0;
        })).catch((error => {
            this.isImagesFinished = !0, this.isLayoutFinished = !0, vutils_1.Logger.getInstance().error("image load failed", error);
    ...
    calculateImageSize(images, imageConfig = {}, ratio = .45) {
        const {imageSizeRange: imageSizeRange, padding: padding = 0} = imageConfig, imageSize = (0, 
        vutils_1.isNumber)(imageConfig.imageSize) ? imageConfig.imageSize : (0, vutils_1.field)(imageConfig.imageSize), size = this.options.size;
        if (imageSize || imageSizeRange) if (imageSize && !(0, vutils_1.isFunction)(imageSize)) images.forEach((img => (0, 
        util_1.setSize)(img, imageSize))); else if (imageSizeRange) {
            const sizeScale = (new vscale_1.SqrtScale).domain((0, vutils_1.extent)(images, (d => d.weight))).range(imageSizeRange);
            images.forEach((img => (0, util_1.setSize)(img, ~~sizeScale.scale(img.weight))));
        } else if (imageSize && (0, vutils_1.isFunction)(imageSize) && !imageSizeRange) {
            const a = .5, [min, max] = (0, vutils_1.extent)(images, (d => d.weight)), picArea = images.reduce(((prev, img) => {
                const r = img.aspectRatio, w = (img.weight - min) / (max - min);
                return prev + (r > 1 ? 1 / r : r) * (a + (1 - a) * w) ** 2;
            }), 0), x = ~~Math.sqrt(ratio * size[0] * size[1] / picArea), range = [ ~~(a * x) - 2 * padding < 0 ? 1 : ~~(a * x) - 2 * padding, ~~x - 2 * padding < 0 ? 1 : ~~x - 2 * padding ], sizeScale = (new vscale_1.SqrtScale).domain((0, 
            vutils_1.extent)(images, (d => d.weight))).range(range);
            images.forEach((img => (0, util_1.setSize)(img, ~~sizeScale.scale(img.weight))));
            ...
            images.forEach((img => (0, util_1.setSize)(img, longSideLength)));
        }
        return images;
    }
    generateTransparentMaskCanvas(shapeImage, size) {
        ...
        this.segmentationOutput.transparentMaskCanvas = transparentMaskCanvas, (null === (_a = this.options.maskConfig) || void 0 === _a ? void 0 : _a.removeWhiteBorder) && (0, 
        image_1.removeBorder)(shapeImage, transparentMaskCanvas, this.segmentationInput.isEmptyPixel);
